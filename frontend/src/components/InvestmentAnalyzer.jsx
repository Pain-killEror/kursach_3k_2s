 ...
 *
 * Props:
 *   object - объект недвижимости из БД
 *   taxRates - налоговые ставки с бэкенда (по entityType пользователя)
 *   formatPrice - функция форматирования цены из CurrencyContext
 *   currency - текущая валюта
 *   convertPrice - функция конвертации
 */
const InvestmentAnalyzer = ({ object, taxRates, formatPrice, currency, convertPrice }) => {
    // Маппинг category из БД (uppercase) в формат для калькулятора
    const CATEGORY_MAP = {
        'КВАРТИРА': 'Квартира',
        'ДОМ': 'Дом',
        'УЧАСТОК': 'Участок',
    ...
    const [infoModal, setInfoModal] = useState(null); // { name, desc }
    ...
    const results = useMemo(() => {
        const commonParams = {
            price, repairCost: Number(repairCost), furnitureCost: Number(furnitureCost),
            legalFeesPct: Number(legalFeesPct),
            useMortgage, downPaymentPct: Number(downPaymentPct),
            mortgageRate: Number(mortgageRate), mortgageTerm: Number(mortgageTerm),
            investmentHorizon: Number(investmentHorizon),
            appreciationRate: Number(appreciationRate),
            useLegalUSN,
        };

        if (strategy === 'LONG_RENT') {
            return calculateRentalAnalysis({
                ...commonParams,
                monthlyRent: Number(monthlyRent), vacancyRate: Number(vacancyRate),
                isShortRent: false,
                maintenancePct: Number(maintenancePct), utilityCost: Number(utilityCost),
                insuranceCost: Number(insuranceCost), managementFeePct: Number(managementFeePct),
            }, taxRates);
        }
        if (strategy === 'SHORT_RENT') {
            return calculateRentalAnalysis({
                ...commonParams,
                monthlyRent: Number(dailyRate) * 30 * (Number(occupancyRate) / 100),
                vacancyRate: 0,
                isShortRent: true,
                dailyRate: Number(dailyRate), occupancyRate: Number(occupancyRate),
                cleaningCost: Number(cleaningCost), platformFeePct: Number(platformFeePct),
                maintenancePct: Number(maintenancePct), utilityCost: Number(utilityCost),
                insuranceCost: Number(insuranceCost), managementFeePct: Number(managementFeePct),
            }, taxRates);
        }
        if (strategy === 'FLIP') {
            return calculateFlipAnalysis({
                ...commonParams,
                expectedSalePrice: Number(expectedSalePrice),
                flipDurationMonths: Number(flipDurationMonths),
                agentFeePct: Number(agentFeePct),
            }, taxRates);
        }
        if (strategy === 'BUY_HOLD') {
            return calculateBuyAndHold(commonParams, taxRates);
        }
        if (strategy === 'BUILD_SELL') {
            return calculateBuildAndSell({
                ...commonParams,
                constructionCost: Number(constructionCost),
                expectedSalePrice: Number(expectedSalePrice),
                buildSellDuration: Number(buildSellDuration),
                agentFeePct: Number(agentFeePct),
            }, taxRates);
        }
        return null;
    }, [strategy, price, repairCost, furnitureCost, legalFeesPct, useMortgage, downPaymentPct,
        mortgageRate, mortgageTerm, monthlyRent, vacancyRate, dailyRate, occupancyRate,
        ...
        insuranceCost, managementFeePct, investmentHorizon, useLegalUSN, taxRates]);

    // Ипотечная сводка
    const mortgageInfo = useMemo(() => {
        if (!useMortgage) return null;
        return calculateMortgage(price, Number(downPaymentPct), Number(mortgageRate), Number(mortgageTerm));
    }, [useMortgage, price, downPaymentPct, mortgageRate, mortgageTerm]);
    ...
    return (
        <div className="inv-analyzer">
            ...
            {/* ========== СЕКЦИЯ 5: РЕЗУЛЬТАТЫ ========== */}
            {results && (
                <div className="inv-results-dashboard">
