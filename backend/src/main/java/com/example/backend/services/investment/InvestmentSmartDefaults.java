...
public class InvestmentSmartDefaults {
    ...
    public InvestmentCalculationRequest enrichWithDefaults(InvestmentCalculationRequest req, RealEstateObject obj) {
        ...
        BigDecimal pricePerM2 = price.divide(area, 2, RoundingMode.HALF_UP);
        ...
        if ("Квартира".equals(type)) {
            ...
            String purpose = getStringAttr(attrs, "land_purpose", "ИЖС");
            defAppreciationRate = "Коммерция".equals(purpose) ? new BigDecimal("8") : new BigDecimal("5");

            int buildCostPerM2 = "ИЖС".equals(purpose) ? 700 : 500;
            defConstructionCost = new BigDecimal(buildCostPerM2 * 120);
            ...
            if (!getBooleanAttr(attrs, "has_electricity", true)) connectionCost += 2000;
            if (!getBooleanAttr(attrs, "has_gas", true)) connectionCost += 3000;
            defRepairCost = new BigDecimal(connectionCost);
